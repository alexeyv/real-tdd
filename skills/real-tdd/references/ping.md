...
2. Start the run.
   ...
   3. Read the task again, line by line, against the list. Every sentence
      ...
      sentence. Both sessions will treat this list as binding.
   4. If no test can run yet, add the minimum that lets one test file
      execute, and commit it on its own. No production code.
   5. Do step 3.5 below, then step 3.6.

3. Be ping as follows. Repeat from step 3.1 until the baton
   reads `done`.

   1. **Wait.** Run this until it returns; if the tool times out, run it
      again.

      ```bash
      until [ "$(cat .real-tdd/baton 2>/dev/null)" = "ping" ] || [ "$(cat .real-tdd/baton 2>/dev/null)" = "done" ]; do sleep 5; done
      ```
   ...
   2. **Receive.** Read the last entry of `.real-tdd/journal.md`,
      ...
      Expect one failing test, uncommitted; the diff is your assignment.
      If the diff is a whole new test file, your partner forgot to commit
      it earlier and it goes into your commit.
      ...
      - If the test asserts something that contradicts `.real-tdd/task.md`,
        ...
        green, and go to step 3.1.
      - If the failing test is one you wrote and your partner handed it
        back, skip to step 3.5 and split it (Child Test), rewrite it, or
        withdraw it and pick another item.

   3. **Green.** Make the failing test pass and keep every other test
      passing, by Fake It, Obvious Implementation, or Triangulate. Choose
      by how sure you are. Write no code that no current test demands; put
      the need on the list instead. If your green also satisfies other list
      items, name them in the journal and check them off as "satisfied by
      <commit>, not driven". After three failed attempts, revert to the
      last green commit, write in the journal that the step was too big,
      hand the baton back with the test still red, and go to step 3.1.
   ...
   5. **Red.** Write the next failing test. You are deciding the interface
      ...
      1. Pick one item from the list (One Step Test). If it is too big,
         split it on the list and take the smallest part.
      2. Assert First, with Evident Data. One behavior per test; the name
         says what it demonstrates.
      3. Isolated Test.
      4. Run the whole suite. The new test must fail and you must be able
         to say why. Not compiling is a valid failure. If it passes, either
         it records existing behavior, so keep it as a Regression Test and
         pick another item, or the code does more than its tests say. Check
         which; a passing new test has found a bug this way.
      ...
      If the list is empty, write `done` to the baton, make the final
      journal entry, and stop. The human decides whether the list was
      finished.

   6. **Hand off.** Append the journal entry in the format below, then
      write `pong` to `.real-tdd/baton`. In that order.

4. Rules that hold throughout.
   - No plan. Write nowhere how the code will be structured.
   - The journal reports. It does not tell your partner what to write or
     how to pass what you wrote. The only instruction between sessions is a
     test.
...
5. Journal entry format. Take the timestamp from `date -u`; do not write
   it from memory.

   ```markdown
   ## Turn N — ping — <output of date -u>
