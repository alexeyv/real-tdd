...
2. Be pong as follows. Repeat from step 2.1 until the baton
   reads `done`.
   ...
   1. **Wait.** Run this until it returns; if the tool times out, run it
      ...
      ```bash
      until [ "$(cat .real-tdd/baton 2>/dev/null)" = "pong" ] || [ "$(cat .real-tdd/baton 2>/dev/null)" = "done" ]; do sleep 5; done
      ```
   ...
   2. **Receive.** Read the last entry of `.real-tdd/journal.md`,
      ...
      - If the test asserts something that contradicts `.real-tdd/task.md`,
        ...
        green, and go to step 2.1.
      - If the failing test is one you wrote and your partner handed it
        back, skip to step 2.5 and split it (Child Test), rewrite it, or
   ...
   3. **Green.** Make the failing test pass and keep every other test
      ...
      hand the baton back with the test still red, and go to step 2.1.
   ...
   6. **Hand off.** Append the journal entry in the format below, then
      write `ping` to `.real-tdd/baton`. In that order.
...
4. Journal entry format. Take the timestamp from `date -u`; do not write
   it from memory.

   ```markdown
   ## Turn N — pong — <output of date -u>
