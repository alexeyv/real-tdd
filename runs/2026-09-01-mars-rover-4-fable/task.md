...
## Clarification from the human, after turn 1

Backward over a pole: the rover's heading flips, the same as on a
forward crossing. A rover on the north pole row heading S that executes
`B` ends on the north pole row at the far longitude heading N. Its back
was toward the pole going in and is away from the pole coming out, so
its front now faces the pole. The end state is not identical to the
forward move in the opposite heading; the mechanism is. This is the
reading the test list should carry.
